...
load_dotenv()
API_DNS = os.getenv("API_DNS") # DNS da API 
API_TOKEN_FILIAL1 = os.getenv("API_TOKEN") # Token de autenticação da API

if not API_DNS or not API_TOKEN_FILIAL1:
    raise ValueError("[ERRO] As variáveis de ambiente API_DNS e API_TOKEN devem ser definidas no arquivo .env.")

# Configurações da API
END_POINT = os.getenv("END_POINT_SALES") # URL da API para obter os dados de vendas
...
params = {
    "idBranch": "0", # Filtrar por ID de filial (0 para todas as filiais)
    "take": 100, # Número de registros a serem retornados por requisição
    "skip": 0,   # Número de registros a serem pulados (para paginação)
    "dateSaleEnd": date_end, # Sempre filtra até o momento atual
}

# Aplica o filtro de data inicial apenas em cargas incrementais
if date_start:
    params["dateSaleStart"] = date_start
...
while True:
    ...
    try:
        response = requests.get( # Faz a requisição GET para a API
            END_POINT, # type: ignore
            params=params,
            auth=HTTPBasicAuth(API_DNS, API_TOKEN_FILIAL1), # Autenticação básica usando o token
...
if "idsale" not in df_sales.columns:
    print("[AVISO] Coluna 'idsale' não encontrada. Deduplicação por ID ignorada.")
else:     # camada extra de segurança com log)
    df_sales.drop_duplicates(subset=["idsale"], inplace=True)
    print("[DROP] Registros duplicados por 'idsale' removidos.")
    
    if os.path.exists(arquivo):
        df_old_ids = pd.read_csv(arquivo, sep=";", usecols=["idsale"])
        qtd_antes = len(df_sales)
        df_sales = df_sales[~df_sales["idsale"].isin(df_old_ids["idsale"])]
